...
- id: bucket_copy 
  ...
  script: |
    ...
    gsutil -m cp -R gs://spls/arc-genai-chat gs://${PROJECT_ID}-bucket
    gsutil -m cp -R gs://spls/arc-genai-chat/Dockerfile /workspace 
    gsutil -m cp -R gs://spls/arc-genai-chat/nginx/default.conf /workspace 
    gsutil -m cp -R gs://spls/arc-genai-chat/nginx /workspace 
...
- id: bucket_config
  ...
  script: |
    ...
    gsutil mv gs://${PROJECT_ID}-bucket/tasks.json gs://${PROJECT_ID}-bucket/arc-genai-chat/tasks.json
    ## SCRIPT END
- id: image_build 
...
substitutions:
  ...
  _IMAGE_NAME: arcade-frontend-chat 
