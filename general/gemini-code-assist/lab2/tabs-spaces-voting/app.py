...
def init_db() -> sqlalchemy.engine.base.Engine:
    ...
    if db is None:
        try:
            ...
            logger.exception(e)
            logger.error("Unable to initalize database connection")
...
def render_index() -> str:
    ...
    if db is None:
        return render_template("error.html", error_msg="Database connection failed.")
    ...
    try:
        ...
        app.logger.error(e)
        return render_template("error.html", error_msg="Unable to start the Voting App. Very sorry about that.")
...
def get_index_context(db: sqlalchemy.engine.base.Engine) -> dict:
    ...
    with db.connect() as conn:
        ...
        # Count number of votes for tabs as tab_count
        tab_count = conn.execute(stmt, parameters={"candidate": "TABS"}).scalar()
        # Count number of votes for spaces as space_count
        space_count = conn.execute(stmt, parameters={"candidate": "SPACES"}).scalar()

        app.logger.debug(f"Current vote counts: TABS={tab_count}, SPACES={space_count}")
    ...
    return {
        ...
        "tab_count": tab_count,
        "recent_votes": votes,
        "explanation": get_vote_explanation(space_count, tab_count),
    }
