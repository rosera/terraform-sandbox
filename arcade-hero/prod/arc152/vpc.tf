...
module "vpc_network_create" {
  ...
  image         = var.gcp_icon 
  resource_name = "${var.gcp_vpc1_name}"

  challenges = [
    { step = "Create a custom VPC network", command = "" },
    { step = "Name the VPC network as \"${var.gcp_vpc1_name}\".", command = "${var.gcp_vpc1_name}" },
  ...
  instructions = [
    ...
    { "step": "Enter the name \"${var.gcp_vpc1_name}\" for your new VPC network.", "command": "${var.gcp_vpc1_name}" },
...
module "vpc_subnet_create" {
  ...
  image         = var.gcp_icon 
  resource_name = "${var.gcp_vpc2_name}"

  challenges = [
    { step = "Name the custom VPC network as \"${var.gcp_vpc2_name}\".", command = "${var.gcp_vpc2_name}" },
    { step = "Create a subnet as \"${var.gcp_subnet1_name}\".", command = "${var.gcp_subnet1_name}" },
    { step = "Set the subnet region: \"${var.gcp_region}\"", "command": "${var.gcp_region}" }, 
    { step = "Add a valid CIDR block for the subnet (e.g., ${var.gcp_subnet1_cidr}).", "command": "${var.gcp_subnet1_cidr}" }, 
    { step = "Do not add any Firewall rules.", command = "" },
    # ... more challenges
  ]
  ...
  instructions = [
    ...
    { "step": "Navigate to the VPC Networks section within your Google Cloud project.", "command": "" },
    { "step": "Click \"CREATE VPC NETWORK\". This button will initiate the VPC network creation process.", "command": "" },
    { "step": "Enter the name \"${var.gcp_vpc2_name}\" for your new VPC network.", "command": "${var.gcp_vpc2_name}" },
    { "step": "Select \"Custom\" subnet creation mode. This option allows you to define your own subnets and their IP address ranges.", "command": "" },
    # { "step": "Select the subnet delete icon to remove this configuration from the VPC.", "command": "" },
    # { "step": "Create a subnet:**  Provide the following details:", "command": "" }, 
    { "step": "Enter the subnet name as \"${var.gcp_subnet1_name}\" for your new subnetwork.", "command": "${var.gcp_subnet1_name}" },
