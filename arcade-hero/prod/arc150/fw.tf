...
module "fw_sourcetag_create" {
  ...
  image         = var.gcp_icon 
  resource_name = "${var.gcp_fw1_name}"

  challenges = [
    { step = "Click the 'CREATE FIREWALL RULE' button.", command = "" },
    { step = "Name the firewall rule a descriptive name (e.g., '${var.gcp_fw1_name}').", command = "${var.gcp_fw1_name}" },
    { step = "Network: Ensure the 'default' VPC network is selected.", command = "" },
    { step = "Direction of traffic: Select '${var.gcp_direction1_name}' for traffic direction.", command = "${var.gcp_direction1_name}" },
    ...
    { step = "Add the target: \"${var.gcp_target1_tag}\".", command = "${var.gcp_target1_tag}" },
    { step = "Source IP ranges: Enter the IP address range (in CIDR notation) to allow traffic from (e.g., '${var.gcp_source1_cidr}' for all addresses).", command = "${var.gcp_source1_cidr}" },
